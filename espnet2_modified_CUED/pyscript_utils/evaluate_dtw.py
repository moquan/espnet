...
"""Evaluate DTW distance between generated and groundtruth audios."""
...
def calculate(
    ...
    for i, gen_path in enumerate(file_list):
        ...
        # DTW
        distance, path = fastdtw(gen_mcep, gt_mcep, dist=spatial.distance.euclidean)
        # twf = np.array(path).T
        ...
        logging.info(f"{gt_basename} {distance:.4f}")
        dtw_dict[gt_basename] = distance
...
def main():
    ...
    with mp.Manager() as manager:
        dtw_dict = manager.dict()
        processes = []
        for f in file_lists:
            p = mp.Process(target=calculate, args=(f, gt_files, args, dtw_dict))
            p.start()
            processes.append(p)
        ...
        dtw_dict = dict(dtw_dict)

        # calculate statistics
        mean_dtw = np.mean(np.array([v for v in dtw_dict.values()]))
        std_dtw = np.std(np.array([v for v in dtw_dict.values()]))
        logging.info(f"Average: {mean_dtw:.4f} ± {std_dtw:.4f}")

    # write results
    if args.outdir is not None:
        os.makedirs(args.outdir, exist_ok=True)
        with open(f"{args.outdir}/utt2dtw", "w") as f:
            for utt_id in sorted(dtw_dict.keys()):
                dtw = dtw_dict[utt_id]
    ...
    logging.info("Successfully finished DTW distance evaluation.")
